...
int main() {
    ...
    world.add(std::make_shared<sphere>(vec3(-6,1,-5), 2, vec3(1.0, 0.0, 1.0)));
    ...
    world.add(std::make_shared<plane>(vec3(0, 1, 0), vec3(0, -1,-1), 5, 5));
    world.add(std::make_shared<plane>(vec3(-1, .5, 2), vec3(5,1,-7), 3, 3));

    camera cam(360, 240, vec3(-1.0, 1.0, 1.0), unit_vec3(vec3(-1.0,0.0,-1.0)), .8);
