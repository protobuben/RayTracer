...
struct plane : public hittable {
    ...
    vec3 coefs, norm, origin, color;
    double displacement;
    double size_x, size_y;
    vec3 u, v;
    
    plane(const vec3& c, vec3 o, double x, double y, vec3 col = vec3(1.0,1.0,1.0)) : coefs(c), norm(unit_vec3(c)), origin(o), size_x(x), size_y(y), color(col) {
    ...
    bool hit(const ray& r, double t_min, double t_max, hit_record& rec) const override {
        const double numenator = displacement-dot(coefs, r.origin);
        const double denominator = dot(coefs, r.direction);
        if (std::fabs(denominator) < 1e-8) { return false; }
        double t = numenator/denominator;
